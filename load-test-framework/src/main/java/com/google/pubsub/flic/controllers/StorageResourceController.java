...
public class StorageResourceController extends ResourceController {
    ...
    private void createStorageBucket() throws IOException {
        try {
            storage.buckets().insert(project, new Bucket()
                    .setName(bucketName(project))).execute();
    ...
    private void uploadFile(Path filePath) throws IOException {
        try {
            ...
            storage
                    .objects()
                    .delete(bucketName(project), filePath.getFileName().toString())
                    .execute();
        } catch (GoogleJsonResponseException e) {
        ...
        storage
                .objects()
                .insert(bucketName(project), null,
