...
import com.google.pubsub.flic.controllers.mode.ModeOverrides;
import com.google.pubsub.flic.controllers.mode.ModeProvider;
import com.google.pubsub.flic.controllers.mode.ModeProviderConverter;
...
public class Driver {
    ...
    private Driver() {
        outputMethods = ImmutableList.of(
                new CsvOutput()
        );
    }
    ...
    public static void main(String[] args) {
        ...
        ModeOverrides overrides = new ModeOverrides();
        JCommander jCommander = JCommander.newBuilder()
                .addObject(driver)
                .addObject(overrides)
                .addObject(KafkaFlags.getInstance()).build();
        jCommander.parse(args);
        if (driver.help) {
            jCommander.usage();
            return;
        }
        if (driver.modeProvider == null) {
            log.error("You must provide a value to the --mode flag to run a loadtest.");
        }
        List<Mode> modes = driver.modeProvider.modes();
        HashMap<Mode, Map<ClientType, LatencyTracker>> results = new HashMap<>();
        modes.forEach(mode -> {
            Mode updated = overrides.apply(mode);
            Map<ClientType, LatencyTracker> result;
            if (overrides.local) {
                result = driver.run(updated,
        ...
        List<ResultsOutput.TrackedResult> trackedResults = results.entrySet().stream().flatMap(entry ->
                entry.getValue().entrySet().stream().map(typeAndTracker -> new ResultsOutput.TrackedResult(
                        entry.getKey(), typeAndTracker.getKey(), typeAndTracker.getValue()))
    ...
    private Map<ClientType, LatencyTracker> run(
            Mode mode, BiFunction<String, Map<ClientParams, Integer>, Controller> controllerFunction) {
        ...
        clientParamsMap.put(params.setClientType(publisherType).build(), mode.numPublisherWorkers());
        clientParamsMap.put(params.setClientType(subscriberType).build(), mode.numSubscriberWorkers());
        try {
    ...
    private void printStats(Mode mode, Map<ClientType, LatencyTracker> trackers) {
        log.info("===============================================");
        trackers.forEach(
                (type, tracker) -> {
                    ...
                    log.info(
                            "Average throughput: "
                                    ...
                                    StatsUtils.getQPS(tracker.getCount(), mode.loadtestDuration()) * mode.messageSize() / 1000000.0)
                                    + " MB/s");
                });
        log.info("===============================================");
    }
