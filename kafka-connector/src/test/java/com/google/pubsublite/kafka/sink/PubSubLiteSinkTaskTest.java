...
public class PubSubLiteSinkTaskTest {
  ...
  /**
   * Tests that Kafka headers are included in the messages published to Pub/Sub Lite but if some of
   * these headers are unsupported either if its key has more than 256 bytes long or its value has
   * more than 1024 bytes it will be discarded.
   */
  @Test
  public void testUnsupportedKafkaHeaders() {
    String veryLongHeaderName = Strings.repeat("-", 257);
    String veryLongValue = Strings.repeat(".", 1025);
    SinkRecord record1 = new SinkRecord(
        KAFKA_TOPIC,
        4,
        Schema.STRING_SCHEMA,
        ...
        50000L,
        TimestampType.CREATE_TIME);
    record1.headers().addString("myHeader", "myValue1");
    record1.headers().addString("myHeader", "myValue2");
    record1.headers().addString(veryLongHeaderName, "anotherValue");
    record1.headers().addString("anotherHeader", veryLongValue);
    record1.headers().addString(veryLongHeaderName, veryLongValue);
    SinkRecord record2 = new SinkRecord(
        KAFKA_TOPIC,
        4,
        Schema.STRING_SCHEMA,
    ...
    Message message1 = expectedBase.toBuilder()
        ...
        .setAttributes(ImmutableListMultimap.<String, ByteString>builder().putAll(attributesBase)
            .put(Constants.KAFKA_OFFSET_HEADER, ByteString.copyFromUtf8("1000"))
            .put(Constants.KAFKA_EVENT_TIME_TYPE_HEADER, ByteString.copyFromUtf8("CreateTime"))
            .put("myHeader", ByteString.copyFromUtf8("myValue1"))
            .put("myHeader", ByteString.copyFromUtf8("myValue2"))
            .build()).build();
    Message message2 = expectedBase.toBuilder()
        .setEventTime(Timestamps.fromMillis(50001))
        .setAttributes(ImmutableListMultimap.<String, ByteString>builder().putAll(attributesBase)
