  ...
  private void runJavaProcess(ClientType.MessagingSide side, Integer port) throws IOException {
    ...
    ProcessBuilder builder =
        new ProcessBuilder(
            ...
            "-Xmx5000M",
            "-cp",
            Client.RESOURCE_DIR + "/driver.jar",
            "com.google.pubsub.clients.gcloud.CPS" + sideInfix + "Task",
            "--port=" + port);
    runCpsProcess(builder);
  }

  private void runPythonProcess(ClientType.MessagingSide side, Integer port)
      throws IOException, InterruptedException {
    File tempDir = unzipToTemp();
    String sideInfix = side == ClientType.MessagingSide.SUBSCRIBER ? "subscriber" : "publisher";
    ProcessBuilder builder =
        new ProcessBuilder("python3", "-m", "clients.cps_" + sideInfix + "_task", "--port=" + port);
    builder.directory(new File(tempDir, "python_src"));
    runCpsProcess(builder);
  }

  private void runNodeProcess(ClientType.MessagingSide side, Integer port)
      throws IOException, InterruptedException {
    File tempDir = unzipToTemp();
    String isPublisher = side == ClientType.MessagingSide.PUBLISHER ? "true" : "false";
    File nodeDir = new File(tempDir, "node_src");
    int installResult = new ProcessBuilder("npm", "install").directory(nodeDir).start().waitFor();
    if (installResult != 0) {
      throw new IOException("Failed to install node project with error code: " + installResult);
    }
    ProcessBuilder builder =
        new ProcessBuilder("node", "src/main.js", "--port=" + port, "--publisher=" + isPublisher);
    builder.directory(nodeDir);
    log.error("node command: " + String.join(", ", builder.command()));
    runCpsProcess(builder);
  }

  private void runGoProcess(ClientType.MessagingSide side, Integer port)
      throws IOException, InterruptedException {
    File tempDir = unzipToTemp();
    String isPublisher = side == ClientType.MessagingSide.PUBLISHER ? "true" : "false";
    ProcessBuilder builder =
        new ProcessBuilder("go", "run", "main.go", "--port=" + port, "--publisher=" + isPublisher);
