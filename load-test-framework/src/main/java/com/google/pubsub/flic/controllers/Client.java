...
public class Client {
    ...
    void start(Timestamp startTime, MessageTracker messageTracker, LatencyTracker latencyTracker) throws Throwable {
        ...
        StartRequest.Builder requestBuilder =
                StartRequest.newBuilder()
                        ...
                        .setTestDuration(Durations.add(params.getMode().loadtestDuration(), params.getMode().burnInDuration()))
                        .setIncludeIds(params.getMode().publishRatePerSec().isPresent());
        if (params.getClientType().isPublisher()) {
            LoadtestProto.PublisherOptions.Builder publisherOptions = LoadtestProto.PublisherOptions.newBuilder()
                    .setMessageSize(params.getMode().messageSize())
                    .setBatchDuration(params.getMode().publishBatchDuration())
                    .setBatchSize(params.getMode().publishBatchSize());
            if (params.getMode().publishRatePerSec().isPresent()) {
                publisherOptions.setRate(params.getMode().publishRatePerSec().get());
            }
            requestBuilder.setPublisherOptions(publisherOptions);
            requestBuilder.setCpuScaling(PUBLISHER_CPU_SCALING);
        } else {
            requestBuilder.setCpuScaling(params.getMode().subscriberCpuScaling());
        ...
        try {
            startFuture.get();
            ScheduledFuture<?> checkFuture = executorService.scheduleAtFixedRate(this::checkClient, 10, 10, TimeUnit.SECONDS);
        ...
        stub.check(
                ...
                new StreamObserver<LoadtestProto.CheckResponse>() {
                    ...
                    public void onNext(LoadtestProto.CheckResponse checkResponse) {
                        ...
                        log.info("Connected to " + params.getClientType() + " client.  Got check response: " + cloned);
                    ...
                    public void onError(Throwable throwable) {
                        if (errors > 3) {
                            clientStatus = ClientStatus.FAILED;
                            doneFuture.setException(throwable);
                            log.error(params.getClientType() + " client failed " + errors
                                    + " health checks, something went wrong.");
                            return;
                        }
                        log.warn("Unable to connect to " + params.getClientType() + " client, probably a transient error.");
                        stub = getStub();
                        errors++;
                    }
